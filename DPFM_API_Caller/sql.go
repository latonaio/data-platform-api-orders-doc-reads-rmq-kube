...
import (
	dpfm_api_input_reader "data-platform-api-orders-doc-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-orders-doc-reads-rmq-kube/DPFM_API_Output_Formatter"
	"fmt"
...
func (c *DPFMAPICaller) HeaderDoc(
	...
	if input.HeaderDoc.OrderID != nil {
		where = fmt.Sprintf("%s\nAND OrderID = %d", where, *input.HeaderDoc.OrderID)
	}
	...
	groupBy := "\nGROUP BY OrderID, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    OrderID, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
...
func (c *DPFMAPICaller) ItemDoc(
	...
	if input.HeaderDoc.ItemDoc.OrderItem != nil {
		where = fmt.Sprintf("%s\nAND OrderItem = %d", where, *input.HeaderDoc.ItemDoc.OrderItem)
	}
	...
	groupBy := "\nGROUP BY OrderID, OrderItem, DocType, DocIssuerBusinessPartner "

	rows, err := c.db.Query(
		`SELECT
    OrderID, OrderItem, DocType, MAX(DocVersionID), DocID, FileExtension, FileName, FilePath, DocIssuerBusinessPartner
		FROM DataPlatformMastersAndTransactionsMysqlKube.data_platform_orders_item_doc_data
		` + where + groupBy + `;`)
	if err != nil {
		*errs = append(*errs, err)
		return nil
